...
var _ = Describe("Storeエンティティを構成する値オブジェクト", Ordered, Label("StoreId構造体の生成"), func() {
	...
	Context("文字数の検証", Label("文字数"), func() {
		It("空文字列の場合、errors.DomainErrorが返る", func() {
			Expect(empty_str).To(Equal(errs.NewDomainError("飲食店IDの長さは36文字でなければなりません。")))
		})
		It("36文字より大きい文字列の場合、errors.DomainErrorが返る", func() {
			Expect(length_over).To(Equal(errs.NewDomainError("飲食店IDの長さは36文字でなければなりません。")))
		})
	})

	Context("UUID形式の検証", Label("UUID形式"), func() {
		It("uuid以外の文字列の場合、errors.DomainErrorが返る", func() {
			Expect(not_uuid).To(Equal(errs.NewDomainError("飲食店IDはUUIDの形式でなければなりません。")))
		})
...
var _ = Describe("Storeエンティティを構成する値オブジェクト", Ordered, Label("StoreName構造体の生成"), func() {
	...
	Context("文字数の検証", Label("無効な文字数"), func() {
		It("空文字列の場合、errors.DomainErrorが返る", func() {
			Expect(empty_str).To(Equal(errs.NewDomainError("飲食店名の長さは5文字以上、30文字以内です。")))
		})
		It("30文字より大きいの場合,errs.DomainErrorが返る", func() {
			Expect(length_over).To(Equal(errs.NewDomainError("飲食店名の長さは5文字以上、30文字以内です。")))
		})
	})
...
var _ = Describe("Storeエンティティを構成する値オブジェクト", Ordered, Label("StorePlace構造体の生成"), func() {
	var empty_str *errs.DomainError    // 空文字列　長さ3文字以上、50文字以内に違反する
	var length_over *errs.DomainError  // 50文字より大きい文字列　長さ3文字以上、50文字以内に違反する
	...
	Context("文字数の検証", Label("無効な文字数"), func() {
		It("空文字列の場合、errors.DomainErrorが返る", func() {
			Expect(empty_str).To(Equal(errs.NewDomainError("飲食店の場所の長さは3文字以上、50文字以内です。")))
		})
		It("50文字より大きいの場合,errs.DomainErrorが返る", func() {
			Expect(length_over).To(Equal(errs.NewDomainError("飲食店の場所の長さは3文字以上、50文字以内です。")))
		})
	})
